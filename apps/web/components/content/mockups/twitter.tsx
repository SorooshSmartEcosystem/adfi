// TwitterMockup — X / Twitter post frame. Avatar + handle + verified
// badge, tweet text (with auto-link styling on URLs and hashtags),
// optional image, engagement row (reply, repost, like, view, bookmark
// + share).

import type { MockupProps } from "./types";
import { pickPrimaryText } from "./types";

export function TwitterMockup({ business, content }: MockupProps) {
  const handle = business.handle ?? business.name.toLowerCase().replace(/\s+/g, "");
  const text = pickPrimaryText(content);
  const tags = content.hashtags ?? [];

  // Render hashtags inline at end if not already in caption
  const inlineTags = tags.length > 0 ? "\n\n" + tags.map((t) => (t.startsWith("#") ? t : `#${t}`)).join(" ") : "";
  const fullText = text + inlineTags;
    ...
    <div className="bg-white border-hairline border-[#cfd9de] rounded-2xl overflow-hidden max-w-[560px] w-full px-lg py-md">
      <div className="flex gap-sm">
        ...
        <div className="flex-1 min-w-0">
          {/* Header line */}
          <div className="flex items-center gap-xs text-[14px] flex-wrap">
