...
### Added
- Landing-page polish — every "get the app" button on adfi.ca jumps straight to `/dashboard` when the visitor is already signed in (was: always sent to `/signup`); the nav CTA relabels to "open dashboard" so signed-in users see where they're going.
