...
- New `@orb/design-agent` package — the start of a per-business "design specialist" pipeline. Two phases ship deterministic (no LLM):
  ...
  - **Phase 5 — application templates**: `renderAllTemplates(input)` substitutes `{{primary}}` / `{{ink}}` / `{{markInner}}` etc. into 5 pre-built layouts (favicon 32×32, social avatar 400×400 circular, business card 856×540, email header 600×200, instagram post 1080×1080). Templates inlined as TS string exports rather than `fs.readFileSync` of `.svg` files — Next.js doesn't trace non-JS assets from workspace packages, which would silently break templates on Vercel.
- Brand kit pipeline now wraps the existing Sonnet+Opus generation with the design-agent layer. Every LLM-generated palette runs through `ensureWcagPalette()`; if any pair fails AA, the palette is auto-corrected by deriving from the LLM's primary as anchor — the LLM keeps its directional taste, the math guarantees readability. The 5 application templates are surfaced via the new `templates` field on `trpc.brandKit.getMine`.
- Brand-kit panel "in context · real-world applications" section now renders the **5 real SVG mockups** from the design-agent's template engine instead of the previous hand-drawn approximations. Adds email-header + instagram-post mockups (net-new). Falls back to the old approximations for legacy kits without a logo `mark`.
- Specialist pages redesigned per `prototype/ADFI_Pulse_Page.html`. Each agent gets its own route (`/specialist/strategist`, `/signal`, `/echo`, `/scout`, `/pulse`, `/ads`) using a shared `SpecialistPageLayout` with a breathing signature orb, tier pill, run-now/pause controls, and a "currently" card that rotates 5–6 phrases with shimmer progress. Sections per agent: BrandVoicePanel for Strategist, FindingsList for Signal/Scout/Pulse, RecentDraftsGrid for Echo, campaigns list / empty-state for Ads.
- Content page redesign prototype at `prototype/ADFI_Content_Page_Redesign.html` — collapsed-by-default sections, amber needs-you card, inline slot expansion (one open at a time), why-this-plan reasoning hidden by default, week calendar grid with state-via-color, performance cards. Standalone HTML; ready to port into the live `/content` route.
- Design-agent skill installed at `.claude/skills/design-agent/SKILL.md` — full 6-phase architectural spec for per-business brand kits / logos / palettes / graphics / templates / voice. Activates Claude Code on related design work.

### Changed (vercel · 2026-04-28)
- Project moved from Vercel Hobby → Pro. `apps/web/vercel.json` re-enables the `functions: { memory: 2048 }` block for the heavy routes (`api/trpc`, `api/cron/*`, `api/upload/logo`) — image-gen and brand-kit logo runs were brushing the 1024MB Hobby ceiling. Sub-daily crons remain off (no current need).

### Changed (brand-kit caps · 2026-04-28)
- Right-sized brand-kit regeneration caps. Old caps were sized for "iterate hard" but real usage is "set up once, leave"; STUDIO and AGENCY were 999 (effectively unlimited), which made worst-case Anthropic spend per user blow through the plan price. New caps per rolling 30 days: TRIAL **0** · SOLO 2 · TEAM 3 · STUDIO 4 · AGENCY 12.
- Brand kit excluded from TRIAL. Each regen costs ~$1.50 in Anthropic spend (Opus 4.7 + adaptive thinking on logos/graphics); granting any to non-paying trialers makes trial-burn a meaningful line item against acquisition cost. Trial users now see a "see plans →" gate on `/brandkit` instead of an empty 0/0 quota line.
- "~$0.30 per regenerate" line in the brand-kit panel was stale (from the original Sonnet-only pipeline). Updated to "~$1.50 per regenerate" — honest typical cost given Opus 4.7 + adaptive thinking. Worst case is ~$3, but typical runs land around $1.20–$1.60 because adaptive thinking rarely maxes the budget.

### Fixed (plan resolution · 2026-04-28)
- **STUDIO trial users were locked out of campaigns + had stale credits**. Three independent bugs combined: (1) `effectivePlan` and `quota.resolvePlanKey` returned `"TRIAL"` for any TRIALING subscription, regardless of the underlying plan — so a STUDIO trialer was treated as TRIAL and `planCanRunCampaigns(TRIAL)` blocked them; (2) all three plan resolvers (`effectivePlan`, `quota.resolvePlanKey`, `billing.getCurrent`) used `findFirst orderBy createdAt desc` — with duplicate subscriptions (an upgrade attempt could spawn a 2nd sub before the active-sub guard landed), the resolver picked whichever was newest, not the highest tier; (3) `UserUsage.creditsLimit` was snapshotted at first touch of the period and never re-read, so if the upgrade webhook missed the cap stayed at the lower plan forever. All three fixed: TRIALING + ACTIVE both count as the sub's plan; resolvers pick highest tier among active/trialing subs; `getCurrentUsage` self-heals on every read by bumping `creditsLimit` up if the resolved plan's cap exceeds the stored cap (never lowers).
- Brand-kit logos rendered as empty colored boxes in the panel even though the downloaded HTML showed them fine. Cause: LLM SVG output reliably includes `viewBox` but often omits `width`/`height` attributes; inside a flex container that itself sizes to content, the SVG collapses to 0×0 (the dimensions go circular). Standalone downloads worked because a bare `<svg>` in the document body inherits the body's width. Fix: `cleanSvg()` now injects `width="100%" height="100%" preserveAspectRatio="xMidYMid meet"` on the root `<svg>` if missing.
- `/onboarding/wow` no longer fails Vercel's prerender pass. The hero handoff via `?biz=` URL param added a `useSearchParams()` call which forces dynamic rendering; the route now wraps `<WowClient />` in `<Suspense>` so Next.js can prerender the static shell.
...
### Fixed
- Critical: Signal no longer leaks the internal "ADFI" name to customers. When a customer DM'd asking "which platform?", Signal had been answering "ADFI" — because the system prompt opened with "You are Signal, the conversational agent inside ADFI" and the thread-history formatter labeled outbound messages as "ADFI:" Both were rewritten so the model speaks AS the business, never as the platform. New rule in the prompt explicitly forbids mentioning ADFI / Signal / the agent / AI / the platform / the system to customers. Business name is now passed in the user message so the model can answer "which platform?" with the actual business name (e.g. SorooshX), and falls back to setting `needsHandoff: true` if the name isn't known.
- Page navigation feels meaningfully snappier. React Query staleTime bumped 30s → 5min, gcTime 30min, refetchOnMount: false. Once-per-request (not once-per-procedure) currentBusinessId resolution. Dashboard reach query reduced 365d → 28d (1Y toggle re-queries client-side when actually clicked). Net effect: clicking between pages with cached data is instant; first paint after cold cache ~50% faster.
- CI typecheck + lint workflows no longer fail on every push. They needed `DATABASE_URL` + `DIRECT_URL` env vars present (even as placeholders) for Prisma's schema-parse to resolve the datasource block. Added job-level placeholder env in `.github/workflows/ci.yml`.
- Vercel deploy schema validation now passes on Hobby plan. Removed the function memory bump (Pro feature) and switched the warmer cron from every-5-min to daily (Hobby cap). For real every-5-min warming, point an external uptime monitor (UptimeRobot / Better Stack) at `https://www.adfi.ca/api/health` — documented in the route file's header.
